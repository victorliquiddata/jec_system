...
def test_nonexistent_user(correlation_id: str):
    ...
    email = cli.prompt_input("Enter non-existent email")
    password = cli.prompt_input("Enter any password", password=True)
...
def test_valid_login(correlation_id: str):
    ...
    email = cli.prompt_input("Enter valid email")
...
def test_invalid_password(correlation_id: str):
    ...
    email = cli.prompt_input("Enter valid email")
    password = cli.prompt_input("Enter wrong password", password=True)
    ...
    try:
        ...
        if auth_manager.login(temp_email, temp_password, correlation_id):
            ...
            if db_user["senha"].startswith("pbkdf2:sha256"):
                ...
                auth_manager.logout()
                if auth_manager.login(temp_email, temp_password, correlation_id):
                    # Final verification of session data
                    user = auth_manager.get_current_user()
...
def test_account_lockout(correlation_id: str):
    ...
    # Get test email
    email = cli.prompt_input("Enter valid email to test lockout")
    ...
    # Phase 1: Test failed attempts
    cli.display_status("\nTesting failed login attempts...", "info")
    for attempt in range(1, 4):  # 1-3 attempts
        start_time = time.time()
        success = auth_manager.login(email, "wrong_password", correlation_id)
        elapsed = time.time() - start_time
    ...
    try:
        ...
        if success:
            ...
            # Verify lockout state in auth_manager
    ...
    # Phase 3: Test lockout expiration (after delay)
    ...
    if hasattr(auth_manager, "lockouts") and email in auth_manager.lockouts:
        ...
        # Should accept login attempts again (but fail due to wrong password)
        success = auth_manager.login(email, "wrong_password", correlation_id)
        if success:
