# test_logger.py - Updated to match current logger implementation
import pytest
import logging
...
def test_log_interface_user_context(logger):
    ...
    with patch.object(logger.loggers["interface"], "info") as mock_log:
        test_user = {"email": "test@example.com", "session_id": "abc123"}
        logger.log_interface("login", "attempt", user_ctx=test_user, level="info")
        # Check that the user info is correctly passed in the extra args
        extra_data = mock_log.call_args[1]["extra"]
        assert extra_data["user_id"] == "test@example.com"
        assert extra_data["session_id"] == "abc123"
...
def test_log_file_rotation(tmp_path):
    ...
    # Verify the component and event fields exist with expected values
    assert log_entry["component"] == "login"
    assert log_entry["event"] == "attempt"
...
def test_multiple_loggers_independence(tmp_path):
    ...
    with patch("logger.Path", return_value=tmp_path):
        ...
        with patch.object(logger1, "log_conexao") as mock1:
            ...
            # Only check for the parameters that are definitely passed
